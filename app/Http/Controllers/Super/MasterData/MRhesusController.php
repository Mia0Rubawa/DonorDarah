...
use App\Models\MasterData\Rhesus;

class MRhesusController extends Controller
{
    //
    ...
    public function __construct()
    {
        ...
        $this->attr = GetAttributeColumnOfEloquentModel::getAttributeColumnOfEloquentModel(new Rhesus, false);
        $this->model = new Rhesus();
        $this->title = 'Rhesus';
    }
    public function index(Request $request)
    {
        ...
        $data['route_data'] = route('super.master_data.rhesus.data');
        $data['route_createOrUpdate'] = route('super.master_data.rhesus.createOrUpdate');
        return view('akun.super.master_data.rhesus', $data);
    }
        ...
        return DataTables::of($this->model->orderBy('id', 'DESC')->select('*'))
            ...
            ->addColumn('action', function ($column) {
                $edit_button = Html::button('Edit')
                    ->attributes(['class' => 'btn btn-warning text-white fw-bolder', 'id' => 'btn_edit', 'data-href' => route('super.master_data.rhesus.show', ['id' => $column->id])]);
                $delete_button = Html::button('Hapus')
                    ->attributes(['class' => 'btn btn-danger text-white fw-bolder ml-3', 'id' => 'btn_delete', 'data-href' => route('super.master_data.rhesus.delete', ['id' => $column->id])]);
                return Html::div(
    ...
    public function show($id)
    {
        ...
        try {
            ...
            if ($find) {
                $find->createOrUpdate = route('super.master_data.rhesus.createOrUpdate', ['id' => $find->id]);
                DB::commit();
                $json['status'] = true;
                $json['data'] = $find;
            } else {
