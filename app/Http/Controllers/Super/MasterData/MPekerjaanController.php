...
class MPekerjaanController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->attr = GetAttributeColumnOfEloquentModel::getAttributeColumnOfEloquentModel(new Pekerjaan, false);
    ...
    public function index(Request $request)
    {
        ...
        $data['route_data'] = route('super.master_data.pekerjaan.data');
        $data['route_createOrUpdate'] = route('super.master_data.pekerjaan.createOrUpdate');
        return view('akun.super.master_data.pekerjaan', $data);
    }
        ...
        return DataTables::of($this->model->orderBy('id', 'DESC')->select('*'))
            ...
            ->addColumn('action', function ($column) {
                $edit_button = Html::button('Edit')
                    ->attributes(['class' => 'btn btn-warning text-white fw-bolder', 'id' => 'btn_edit', 'data-href' => route('super.master_data.pekerjaan.show', ['id' => $column->id])]);
                $delete_button = Html::button('Hapus')
                    ->attributes(['class' => 'btn btn-danger text-white fw-bolder ml-3', 'id' => 'btn_delete', 'data-href' => route('super.master_data.pekerjaan.delete', ['id' => $column->id])]);
                return Html::div(
    ...
    public function show($id)
    {
        ...
        try {
            ...
            if ($find) {
                $find->createOrUpdate = route('super.master_data.pekerjaan.createOrUpdate', ['id' => $find->id]);
                DB::commit();
                $json['status'] = true;
                $json['data'] = $find;
            } else {
